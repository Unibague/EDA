...
Route::get('realTest', function (){
    ...
    foreach ($evaluatedIds as $evaluatedId){
        ...
        //The calculation can be done, now let's verify how many actors submitted the answer and
            // based on that assign the assessment weight for every perspective
            ...
            foreach ($localFormAnswers as $formAnswer) {
                ...
                if (count($localFormAnswers) === 3) {
                //In this scenario, user doesn't have a peer, so that percentage has to be distributed on the client and the boss
                ...
                elseif (count($localFormAnswers) === 4) {

                }

                else {
                    if ($formAnswer->role === "cliente") {
